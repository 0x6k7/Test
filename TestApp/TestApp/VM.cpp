...
VM::VM(void)
{
	...
	{
		Beep(0, 0);
		VM::nextInstructionAlign = 0;
		VM::callTargetAlign = 0;
		VM::instructionDataOffset = 0;
		VM::globalsOffsetAlign = 0;
		VM::stack = (int*)malloc(10 * 4); // Limited to 10 items
		VM::stackCount = 0;
		VM::tempValue = 0;
	}
}
...
void VM::Initialize(void* instructionData, int size)
{
	for (int i = 0; i < 255; i++)
	{
		VM::globalsPtr[i] = 0x0;
...
NoDebugger:
	// Copy data
	VM::instructionData = malloc(size);
	memcpy(VM::instructionData, instructionData, size);
}
...
bool VM::Run()
{
	...
	int nextInstructionCode = nextInstruction->code + VM::nextInstructionAlign;
	...
	{
		// Decode target
		int* globals = VM::globalsPtr[nextInstruction->callContext.globals + VM::globalsOffsetAlign];
		VM::globalsOffsetAlign = 0;
		int targetNumber = nextInstruction->callContext.arguments[0];
		int function = globals[targetNumber + VM::callTargetAlign];
	...
	case -0x5:
	{
		...
		if (hash == nextInstruction->callContext.hash)
		{
			...
			void* tempMemory = malloc(4 + VM::stackCount * 4);
			memcpy(tempMemory, nextInstruction->callContext.arguments, 4);
			memcpy((void*)((int)tempMemory + 0x4), VM::stack, VM::stackCount * 4);
	...
	case -0x6:
	{
		VM::stack[VM::stackCount] = VM::tempValue;
		VM::stackCount++;
	}
	...
	case -0x7:
	{
		...
		VM::globalsOffsetAlign = 0;
		int targetNumber = nextInstruction->callContext.arguments[0];
		VM::tempValue = globals[targetNumber];
...
{
	free(VM::stack);
	free(VM::instructionData);
}

void* VM::GetDataStream()
{
	return VM::instructionData;
}

void VM::AddGlobals(int* globals)
{
	for (int i = 0; i < 255; i++)
	{
		if (VM::globalsPtr[i] == 0x0)
		{
			VM::globalsPtr[i] = globals;
			break;
		}
	}
}

Instruction* VM::GetNextInstruction()
{
	// Allocate 4 bytes for the code (integer)
	void* instructionCode = malloc(4);
	int firstInstruction = 0;
#ifdef OBFUSCATE
	int address = (int)VM::instructionData + VM::instructionDataOffset + GetPEB()->BeingDebugged;
#elif !OBFUSCATE
...
#ifdef OBFUSCATE
#ifndef _DEBUG
		bool (VM::*funcPtr)(void) = &VM::IsDbgPresentPrefixCheck;
...
#ifdef OBFUSCATE
	address = (int)VM::instructionData + VM::instructionDataOffset + VM::IsDbgPresentPrefixCheck() + 4 - 24;
#elif !OBFUSCATE
	address = (int)VM::instructionData + VM::instructionDataOffset + 4 - 24;
#endif
...
void VM::AdjustDataOffset(int offset)
{
	VM::instructionDataOffset += offset;
}

void VM::AdjustInstructionCode(int offset)
{
	VM::nextInstructionAlign += offset;
}

void VM::AdjustCallTarget(int offset)
{
	VM::callTargetAlign += offset;
}

void VM::AdjustGlobalsOffset(int offset)
{
	VM::globalsOffsetAlign += offset;
}
...
__forceinline bool VM::IsDbgPresentPrefixCheck()
{
	__try
	{
